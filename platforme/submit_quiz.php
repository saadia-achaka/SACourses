...
// Récupérer les questions de la catégorie "Développement"
$correct_answers_query = $conn->prepare("
    SELECT id, question_text, correct_answer_id 
    FROM quiz_questions 
    WHERE category_id = ?
");
$correct_answers_query->execute([$category_id]);
...
// Récupérer les réponses possibles pour les questions de cette catégorie
$question_ids = array_keys($correct_answers); // Obtenez les IDs des questions
if (!empty($question_ids)) {
    // Convertir les IDs en une liste pour la clause IN
    ...
    ");
    $answers_query->execute($question_ids);
